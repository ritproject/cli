defmodule RitCLI.CLI.Config.Server.Operations.SetDefault do
  @moduledoc false

  alias RitCLI.CLI.{Config, Output}
  alias RitCLI.CLI.Config.Server.Input

  @spec set_default_server(Input.SetDefault.t(), Config.Server.t()) ::
          {:ok, Config.Server.t()} | {:error, atom(), Output.t()}
  def set_default_server(
        %Input.SetDefault{name: name, path: path},
