defmodule RitCLITest.CLI.Config.Server.EditTest do
  ...
  describe "command: rit config server edit" do
    ...
    test "<server_name>, do: explain about options restriction, exit 1" do
      ...
      error_message = """
      Error: expects at least one of the optional inputs: <server_name>, <server_hostname>, <server_port>
      """
    ...
    test "<server_name> --hostname <invalid_hostname>, do: instruct correct <hostname>, exit 1" do
      ...
      error_message = """
      Error: <server_hostname> argument with value 'http://test.com/I-met-a-traveller-from-an-antique-land-Who-said-Two-vast-and-trunkless-legs-of-stone-Stand-in-the-desert-Near-them-on-the-sand-Half-sunk-a-shattered-visage-lies-whose-frown' must have at most 150 characters
      """
    ...
    test "<server_name> --port <invalid_port>, do: instruct correct <port>, exit 1" do
      ...
      error_message = """
      Error: <server_port> argument with value 'not_a_port' must be a number
      """
