defmodule RitCLI.CLI.Config.Server.Default.Input.Set do
  @moduledoc false

  alias RitCLI.CLI.{Config, Helper, Input, Output}
  alias RitCLI.CLI.Config.Server.Default.Input.Set
  ...
  defp perform_operation(%Set{} = input) do
    with {:ok, %Set{name: name} = input} <- validate(input),
      ...
      {:ok, %Output{message: message, module_id: :config_server_default_set}}
    end
  end
  ...
  defp validate(%Set{} = input) do
    with {:ok, input} <- Validator.validate_name(input),
      ...
      {:error, error} ->
        {
          :error,
          :config_server_default_set_input_error,
          %Output{
            error: error,
            module_id: :config_server_default_set
          }
        }
    end
  end
  ...
  defp arguments_error(argv) do
    ...
    {
      :error,
      :unknown_config_server_default_set_arguments,
      %Output{
        error: "Unknown config server default set arguments: '#{arguments}'",
        module_id: :config_server_default_set,
