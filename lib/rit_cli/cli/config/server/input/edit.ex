defmodule RitCLI.CLI.Config.Server.Input.Edit do
  ...
  defp perform_operation(%Edit{} = input) do
    with {:ok, %Edit{name: name, current_name: current_name} = input} <-
      ...
      {:ok, %Output{message: message, module_id: :config_server_edit}}
    end
  end
  ...
  defp validate(%Edit{} = input) do
    with {:ok, input} <- Validator.validate_name(input, :current_name),
         {:ok, input, optionals} <-
           Input.Validator.optional(input, :name, &Validator.validate_name/2),
         {:ok, input, optionals} <-
           Input.Validator.optional(input, :hostname, &Validator.validate_hostname/2, optionals),
         {:ok, input, optionals} <-
           Input.Validator.optional(input, :port, &Validator.validate_port/2, optionals),
         :ok <-
           Input.Validator.validate_at_least_one_optional(optionals, [:name, :hostname, :port]) do
      {:ok, input}
    else
      {:error, error} ->
        {
